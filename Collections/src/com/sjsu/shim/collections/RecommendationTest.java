...
public class RecommendationTest {
	...
	public void test() {
		...
		List<Book> recommendedBooks=recom.recommend(soldBooks, viewedBooks, wishlistedBooks);
		boolean bestseller=false;
		for(Book b:recommendedBooks)
		{
			//the books sold must be part of best selling books list, most visited booklist or most wishlisted book list.
			//If any book in recommended books does not belong to these 3 lists we set bestseller as false and test case will fail
			if(vb3.contains(b)||sb3.contains(b)||wl2.contains(b))
			{
				bestseller=true;
			}
			assertNotNull(b);
